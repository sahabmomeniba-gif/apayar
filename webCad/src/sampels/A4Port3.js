...
// import {DocsService} from "../../components/services/docsService";
const styles= (theme)=>({
    root:{
        width:'100%',
        height:'97vh',
...
const insertLabelsToLines = (polygon,textLayer,textHeight)=>{
    ...
    let feature =  new Feature;
    let rot = (p1,p2) =>  Math.atan2(p2[1]-p1[1],p2[0]-p1[0])
    let mid = (p1,p2) =>  [(p2[0]+p1[0])/2,(p2[1]+p1[1])/2]

    for (let i = 0; i < coords.length; i++) {
        if(i != coords.length-1){
            feature.setGeometry(new LineString([coords[i],coords[i+1]]))
            if(feature.getGeometry().getLength() > 0){
                let rotaitons = rot(coords[i],coords[i+1])
                let InteriorPoint = mid(coords[i],coords[i+1])
                let tstyle = new TextStyle('20px B-Nazanin')
                let text = new SiText(textLayer.map,'label1',InteriorPoint[0],InteriorPoint[1],rotaitons,''+'24',{TextStyle:tstyle,layer:textLayer})
                text.addTextByTextHeight(textHeight,'mid bottom')
                text.show()
                text.render()
                let label = '24'
                if(label) {
                    // label += adjs[i].doc.fare ? '/' + adjs[i].doc.fare:'';
                    let text2 = new SiText(textLayer.map, 'label1', InteriorPoint[0], InteriorPoint[1], rotaitons, label, {
                        TextStyle: tstyle,
                        layer: textLayer
                    })
                    text2.addTextByTextHeight(textHeight, 'mid top')
                    text2.show()
                    text2.render()
                }
            }
...
class A4Port extends Component {
    ...
    async initMap() {
        ...
        this.siMap = new SiMap(this.mapRef.current, this.commandLine, {
            ...
            controls: {
                ...
                textsPropertiesControl:false,
                layerPropertiesControl:false,
                snapPropertiesControl:false,
            }
        })
    ...
    async a4port(){
        ...
        // const data = await DocsService.serverFunction('getHodud', {doc: this.props.parent.doc})
        // if (!data.adjacentTable)
        //     return;
        // let p  = data.adjacentTable.map(a=>a.lineCoordinates[0])
        // p.push(p[0])
        // let p1 = MapHelper.lonlat2xy_arr(p)
    ...
    async saveToPNG() {
        let allEntity = this.siMap.getAllEntites()
        let extent = this.siMap.calcExtentFromFeaturesCollection(allEntity)
        let img = await this.siMap.exportAsPNG(60, 60, allEntity, extent)
        ...
        let f = await DocsService.callAPI("./api/files/writeFromBase64", {
            type: "image/png",
            name: 'imageMap.png',
            base64String: img.replace("data:", "")
                .replace(/^.+,/, ""),
            // parentId: unit._id,
            fName: "imageMap"
        });

        DocsService.updateOne('centroid', this.props.parent.doc._id, {
            $set: {
                imageMap_id: f.fileId,
    ...
    render() {
       ...
       return <>
               <div dir="ltr" ref={this.mapRef} className={classes.root} style={{height:'600px',float:'right',width:'50%',cursor:'url(cursor.ico) 30 30,auto'}}>
               </div>
