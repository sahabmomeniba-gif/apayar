...
export default class DrawLine extends Command {
    constructor(option) {
        super(option)
        this.name = 'line'
                ...
                if (this.startPoint) {
                    if (!isEqualPoint(this.startPoint, data, 1000)) {
                        ...
                        this.lineSegments.push(entity)
                            // parallelSnap.removeSnapFeature()
                            // parallelSnap.prvsLine = entity

                        // let viewExtent = this.siMap.getViewExtent()
                        // console.log(getExtendPointsToExtent(viewExtent,new Feature({geometry:new LineString([this.startPoint,data])}),this.siMap))
                        // console.log(getExtendPointsToExtent(viewExtent,GetVerticalLine(new Feature({geometry:new LineString([this.startPoint,data])})),this.siMap))                      
                        this.handleNext()
    ...
    onDone() {
        ...
        // console.log(this.lineSegments.getArray())
        let polarTracking = this.siMap.siSnap.snapCollection.find(snap => snap.name === snapTypeName.polarTracking)
        let parallelSnap = this.siMap.siSnap.snapCollection.find(snap => snap.name === snapTypeName.parallel)
        parallelSnap.reset()
        if (polarTracking) polarTracking.removeSnapFeature()
